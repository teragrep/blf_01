...
public class SubTokenSplitTest {
    ...
    public SubTokenSplitTest() {
        tokenSplit = new SubTokenSplit();
    }


    @Test
    public void subTokeSplitTest() {
        String testString = "a#b$c%d-e.f/g:h=i@j\\k_l";
        String[] tokens = tokenSplit.split(testString);
        assertEquals("a", tokens[0]);
        assertEquals("b", tokens[1]);
        assertEquals("c", tokens[2]);
        assertEquals("d", tokens[3]);
        assertEquals("e", tokens[4]);
        assertEquals("f", tokens[5]);
        assertEquals("g", tokens[6]);
        assertEquals("h", tokens[7]);
        assertEquals("i", tokens[8]);
        assertEquals("j", tokens[9]);
        assertEquals("k", tokens[10]);
        assertEquals("l", tokens[11]);
    }
}
